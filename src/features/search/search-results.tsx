...
import EnhancedArtistCard from "@/features/search/components/enhanced-artist-card";
          ...
          {artists.length > 0 ? (
            <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 animate-fade-in-up">
              {artists.map((artist, index) => (
                <Card key={artist.id} className="p-0 border-0">
                  <EnhancedArtistCard artist={artist} index={index} />
                </Card>
              ))}
            </div>
            ...
            <NotFound
              searchQuery={query}
              title={t("no-artist-found")}
              description={t("try-with-other-keywords")}
