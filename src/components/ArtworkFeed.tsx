...
export default function ArtworksFeed({ isAuthenticated = false }: ArtworksFeedProps) {
  ...
  const LoadingSkeleton = () => (
    <div className={`${isAuthenticated ? 'flex gap-4 px-6' : 'grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 px-6'}`}>
    ...
    <section className="py-8 bg-white overflow-hidden">
      ...
      {isAuthenticated ? (
        ...
        <div className="relative group">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-6 bg-gradient-to-r from-white to-transparent z-20 pointer-events-none"></div>
          <div className="absolute right-0 top-0 bottom-0 w-6 bg-gradient-to-l from-white to-transparent z-20 pointer-events-none"></div>
          <div
            ref={scrollRef}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 px-6">
          {artworks.map((artwork, index) => (
            <div
              key={artwork.id}
              className="bg-white border border-gray-100 rounded-xl overflow-hidden cursor-pointer group/card hover:shadow-md transition-all duration-300 hover:-translate-y-1"
              style={{ animationDelay: `${index * 50}ms` }}
            >
